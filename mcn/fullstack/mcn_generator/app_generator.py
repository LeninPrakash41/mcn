...
"""
    ...
    def generate_api(self, resource_name: str, crud: bool = True) -> int:
        """Generate API endpoints for resource"""
        print(f"Generating API for: {resource_name}")

        code = f'''// {resource_name} API - Generated by MCN
...
'''
        ...
        path = backend_dir / f"{resource_name.lower()}_api.mcn"
        self._write(path, code)
        print(f"✓  {path}")
        return 0

    def generate_ui(self, component_name: str, model_name: str = None) -> int:
...
"""
        ...
        path = frontend_dir / f"{component_name}.tsx"
        self._write(path, code)
        print(f"✓  {path}")
        return 0
...
def main():
    ...
    # generate-api
    p_api = sub.add_parser("generate-api", help="Generate API endpoints for a resource")
    p_api.add_argument("name", help="Resource name")
    p_api.add_argument("--crud", action="store_true", help="Include CRUD operations")

    # generate-ui
    p_ui = sub.add_parser("generate-ui", help="Generate React UI component")
    p_ui.add_argument("name", help="Component name")
    p_ui.add_argument("--model", help="Associated model name")
    ...
    if args.command == "generate-api":
        return gen.generate_api(args.name, args.crud)

    if args.command == "generate-ui":
        return gen.generate_ui(args.name, args.model)
