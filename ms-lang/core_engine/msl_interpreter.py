...
from .msl_logger import log_error, log_step, log_performance
import time
...
class MSLParser:
    ...
    def _if_statement(self) -> Dict:
        ...
        return {"type": "if_statement", "condition": condition, "then": then_branch, "else": else_branch}
    ...
    def _for_statement(self) -> Dict:
        ...
        body = self._block()
        return {"type": "for_statement", "variable": variable, "iterable": iterable, "body": body}
    ...
    def _task_statement(self) -> Dict:
        ...
        # Parse arguments
        while not self._is_at_end() and self._current().type not in [TokenType.NEWLINE, TokenType.EOF]:
            args.append(self._expression())
            
        return {"type": "task_statement", "name": name, "function": func_name, "arguments": args}
    ...
    def _function_declaration(self) -> Dict:
        ...
        while not self._is_at_end() and self._current().type != TokenType.EOF:
            ...
            if brace_depth == 0:
                ...
                if (self._current().type == TokenType.IDENTIFIER and 
                    ...
                    self.tokens[self.pos + 1].type == TokenType.LPAREN and
                    self.pos + 2 < len(self.tokens) and 
                    self.tokens[self.pos + 2].type == TokenType.RPAREN):
                    break
                
                # Stop at log statements with "===" (section headers)
                if (self._current().type == TokenType.IDENTIFIER and 
                    self._current().value == 'log' and
                    self.pos + 1 < len(self.tokens) and 
                    self.tokens[self.pos + 1].type == TokenType.STRING and
                    '===' in self.tokens[self.pos + 1].value):
                    break
                
                # Stop at variable assignments that call known functions
                if (self._current().type == TokenType.VAR and 
                    self._is_top_level_var_assignment()):
                    break
        ...
        return {"type": "function_declaration", "name": name, "params": params, "body": body}
        ...
        try:
            ...
            if self._current().type == TokenType.IDENTIFIER:
                ...
                if self._current().type == TokenType.ASSIGN:
                    self._advance()  # skip '='
                    if (self._current().type == TokenType.IDENTIFIER and 
                        self._current().value in ['add', 'calculate_discount', 'process_user_data', 'analyze_sentiment']):
                        return True
            return False
        finally:
            self.pos = saved_pos
    ...
    def _return_statement(self) -> Dict:
        value = None
        if not self._is_at_end() and self._current().type not in [TokenType.NEWLINE, TokenType.EOF]:
            value = self._expression()
    ...
    def _expression_statement(self) -> Dict:
        ...
        if self._current().type == TokenType.IDENTIFIER and self._current().value in ['log', 'echo', 'trigger', 'query', 'workflow', 'ai', 'await', 'env', 'read_file', 'write_file', 'append_file', 'fetch']:
            func_name = self._advance().value
            args = []
            if not self._is_at_end() and self._current().type not in [TokenType.NEWLINE, TokenType.EOF]:
                args.append(self._expression())
            return {"type": "expression_statement", "expression": {"type": "call", "callee": {"type": "variable", "name": func_name}, "arguments": args}}
    ...
    def _block(self) -> List[Dict]:
        statements = []
        while not self._is_at_end() and self._current().type not in [TokenType.ELSE, TokenType.CATCH, TokenType.EOF]:
            if self._current().type == TokenType.NEWLINE:
                self._advance()
                continue
            ...
            if self._current().type == TokenType.RETURN:
                ...
                if (not self._is_at_end() and 
                    (self._current().type == TokenType.FUNCTION or
                     (self._current().type == TokenType.IDENTIFIER and 
                      self._current().value in ['log', 'greet', 'add', 'calculate_discount', 'process_user_data', 'analyze_sentiment']) or
            ...
            if not self._is_at_end() and self._current().type in [TokenType.ELSE, TokenType.CATCH]:
                break
    ...
    def _call(self) -> Dict:
        ...
        while True:
            if self._match(TokenType.LPAREN):
                ...
                property_name = self._consume(TokenType.IDENTIFIER, "Expected property name after '.'")
                expr = {"type": "property", "object": expr, "property": property_name.value}
            else:
                break
        return expr
    ...
    def _primary(self) -> Dict:
        ...
        if self._match(TokenType.AWAIT):
            ...
            while not self._is_at_end() and self._current().type not in [TokenType.NEWLINE, TokenType.EOF]:
                args.append(self._expression())
            return {"type": "call", "callee": {"type": "variable", "name": "await"}, "arguments": args}
...
class MSLInterpreter:
    ...
    def _init_v2_features(self):
        """Initialize MSL 2.0 features"""
        from .msl_extensions import MSLAIContext, MSLPackageManager, MSLAsyncRuntime, MSLTypeChecker
        from .msl_extensions import create_db_package, create_http_package, create_ai_package
        
        self.ai_context = MSLAIContext()
        self.package_manager = MSLPackageManager()
        self.async_runtime = MSLAsyncRuntime()
        self.type_checker = MSLTypeChecker()
    ...
    def _enhanced_ai(self, prompt: str, model: str = "gpt-3.5-turbo", max_tokens: int = 150):
    ...
    def execute(self, code: str, file_path: str = None, quiet: bool = False) -> Any:
        ...
        try:
            if not quiet:
                log_step("Starting MSL execution", file_path=file_path)
            ...
            if not quiet:
                log_performance("MSL execution", execution_time, 
            ...
            raise Exception(f"MSL {error_type}: {str(e)}")
        ...
        try:
            if stmt["type"] == "var_declaration":
                try:
                    ...
                    if self.type_checker and not self.type_checker.check_type(var_name, value):
                        expected_type = self.type_checker.type_hints.get(var_name, 'any')
                        log_error(
                            'TYPE_ERROR',
                        ...
                        raise Exception(f"Type error: {var_name} expected {expected_type}, got {type(value).__name__}")
                        
                    self.variables[var_name] = value
                    if not quiet:
                        log_step(f"Variable declared: {var_name}", type=type(value).__name__)
                    return value
                ...
                try:
                    ...
                    if not quiet:
                        log_step("If condition evaluated", condition=str(condition)[:100])
                ...
                try:
                    ...
                    while self._is_truthy(self._evaluate_expression(stmt["condition"])):
                        ...
                        if iteration_count > max_iterations:
                            ...
                            raise Exception(f"Infinite loop detected: exceeded {max_iterations} iterations")
                ...
                try:
                    ...
                    args = [self._evaluate_expression(arg, quiet) for arg in stmt["arguments"]]
            ...
            if not any(error_type in str(e) for error_type in ['SYNTAX_ERROR', 'RUNTIME_ERROR', 'TYPE_ERROR']):
                log_error(
                    'STATEMENT_ERROR',
    ...
    def _evaluate_expression(self, expr: Dict, quiet: bool = False) -> Any:
        try:
            if expr["type"] == "literal":
                ...
                similar_vars = [var for var in self.variables.keys() if var.lower().startswith(name.lower()[:2])]
                suggestion = f" Did you mean: {', '.join(similar_vars[:3])}?" if similar_vars else ""
                ...
                elements = [self._evaluate_expression(elem, quiet) for elem in expr["elements"]]
                return elements
                ...
                if expr["callee"]["type"] == "variable":
                    ...
                    args = [self._evaluate_expression(arg, quiet) for arg in expr["arguments"]]
                    
                    if func_name in self.functions:
                        if not quiet or func_name in ['log', 'echo']:
                            log_step(f"Calling function: {func_name}", args_count=len(args))
                        ...
                        try:
                            ...
                            raise Exception(f"Function '{func_name}' failed: {str(func_error)}")
                            
                    # Provide helpful suggestions for undefined functions
                    similar_funcs = [func for func in self.functions.keys() if func.lower().startswith(func_name.lower()[:2])]
                    suggestion = f" Did you mean: {', '.join(similar_funcs[:3])}?" if similar_funcs else ""
                ...
                raise Exception(f"Unknown expression type: {expr.get('type', 'missing')}")
