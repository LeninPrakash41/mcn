"""
MSL Frontend Integration Module
Helps developers handle frontend applications with MSL backend services
"""
...
from .msl_logger import log_error, log_step

class MSLFrontendIntegration:
    """Handles frontend application integration with MSL backend"""
    ...
    def generate_api_client(self, msl_endpoints: List[Dict[str, Any]], 
                          framework: str = None) -> str:
        """Generate frontend API client code for MSL endpoints"""
        ...
        return generator(msl_endpoints)
...
class MSLApiClient {
  constructor(baseURL = 'http://localhost:8080') {
    ...
    this.client.interceptors.response.use(
      (response) => response,
      (error) => {
        console.error('MSL API Error:', error.response?.data || error.message);
...
'''
  ...
  async {method_name}(data = {{}}) {{
    try {{
      ...
      throw new Error(`MSL {method_name} failed: ${{error.message}}`);
    }}
  }}
'''
...
export default MSLApiClient;

// React Hook for MSL API
import { useState, useCallback } from 'react';

export const useMSLApi = () => {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const client = new MSLApiClient();
...
export default {
  install(app) {
    const mslClient = new MSLApiClient();
    app.config.globalProperties.$msl = mslClient;
    app.provide('msl', mslClient);
  }
};
...
class MSLApiClient {
  ...
  async request(endpoint, data = {}, method = 'POST') {
    ...
    try {
      ...
      console.error('MSL API Error:', error);
      throw error;
    }
  }
'''
...
'''
    ...
    def create_frontend_config(self, framework: str, msl_endpoints: List[Dict[str, Any]]):
        """Create frontend configuration files"""
        
        log_step(f"Creating frontend config for {framework}")
        
        # Create API client file
        client_code = self.generate_api_client(msl_endpoints, framework)
        ...
        client_file = self.project_path / f'msl-api-client.{ext}'
        ...
        env_config = {
            ...
            'MSL_ENDPOINTS': {ep.get('name'): ep.get('path') for ep in msl_endpoints}
        }
        
        env_file = self.project_path / '.env.msl'
    ...
    def generate_frontend_examples(self, framework: str, endpoints: List[Dict[str, Any]]) -> Dict[str, str]:
        """Generate example frontend code for using MSL APIs"""
...
msl_frontend = MSLFrontendIntegration()