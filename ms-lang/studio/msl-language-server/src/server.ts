...
// MSL built-in functions for completion
const mslBuiltins: CompletionItem[] = [
  {
    label: 'log',
  ...
  },
  {
    ...
    data: 5,
    detail: 'use("package")',
    documentation: 'Load MSL package functions'
  },
  {
    label: 'task',
    kind: CompletionItemKind.Function,
...
// MSL keywords
const mslKeywords: CompletionItem[] = [
  {
    label: 'var',
...
connection.onCompletion(
  (_textDocumentPosition: TextDocumentPositionParams): CompletionItem[] => {
    return [...mslBuiltins, ...mslKeywords];
  }
);
...
connection.onCompletionResolve(
  (item: CompletionItem): CompletionItem => {
    if (item.data === 1) {
      item.detail = 'MSL built-in function';
...
connection.languages.onDocumentDiagnostic(async (params) => {
  ...
  if (document !== undefined) {
    return {
      ...
      items: validateMSLDocument(document)
...
function validateMSLDocument(textDocument: TextDocument) {
  const text = textDocument.getText();
  const diagnostics = [];
  
  // Basic MSL syntax validation
  const lines = text.split('\n');
  
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i].trim();
    
    // Check for common MSL syntax errors
